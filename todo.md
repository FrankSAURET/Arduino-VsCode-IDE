...
8. ✅ **Publication faite par Frank** le 19 septembre 2026, hors de cette session. La `2026.9.4` est en ligne : sa section du CHANGELOG est datée, donc close — les prochains lots iront sous une nouvelle section.
9. ✅ **Essais du lot `.35` validés par Frank** : numéro de lot affiché sur la page d'accueil, et désinstallation qui efface bien le dossier de l'extension. Le correctif du journal ([logger.ts](src/logger/logger.ts)) est confirmé à l'exécution.
10. ℹ️ **`buildNumber` laissé à `2026.9.4.36`** : son préfixe doit reprendre la version **déjà publiée**, ce qui est désormais le cas. Le prochain lot livré passera à `2026.9.4.37`.
...
12. ✅ **Contrôle du `buildNumber` assoupli** ([scripts/package.mjs](scripts/package.mjs)) : il exigeait que `buildNumber` commence exactement par `version`. Il valide désormais la forme `ANNÉE.MOIS.incrément.compteur` et ne refuse qu'un vrai retour en arrière, comparé segment par segment au premier qui diffère (sinon `2026.10.0` face à `2026.9.9` passait pour un recul). Le cas normal reste l'égalité des préfixes. 10/10 aux essais.
13. ✅ **Règle de numérotation confirmée par Frank** : le `buildNumber` se préfixe de la version **déjà publiée** (`version`), pas de la prochaine. Les deux n'avancent qu'à la publication, pour que le paquet installé en local se fasse bien remplacer par la mise à jour en ligne. Lot renuméroté `2026.9.3.32` (et `.31` pour le précédent).
14. ✅ **Paquet construit et vérifié** : `arduino-vscode-ide-2026.9.3.32.vsix`. Contenu contrôlé après décompression — empreinte des `#include`, déclencheur `onDidSaveTextDocument`, `build(BuildMode.Analyze, undefined, true)` dans l'analyse automatique **et** dans la commande manuelle, avertissement d'échec, traduction FR.
15. ⏳ **Non vérifié à l'exécution** : pas d'essai dans un éditeur réel. À confirmer par Frank — installer le `.vsix`, lancer une fois Ctrl+Alt+I (amorçage de l'empreinte), puis ajouter un `#include` dans ds18b20-uno, enregistrer, attendre ~5 s.

# v2026.9.3.31 — Reconstruire IntelliSense ne reconstruisait plus rien
