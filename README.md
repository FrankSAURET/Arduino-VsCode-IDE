...
Open VS Code and press F1 or Ctrl + Shift + P *or* Cmd + Shift + P to open command palette, select **Install Extension** and type `arduino-vscode-ide`.
...
Keybindings: Alt + Cmd + I *or* Alt + Ctrl + I
...
1. Static contribution strings (command titles, view names, etc.) use `package.nls.json` for English defaults. To translate those, create `package.nls.<locale>.json` at the repository root following the same key structure.
