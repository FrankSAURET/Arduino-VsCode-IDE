...
- **Traceur série sur VSCodium et les autres éditeurs** : l'extension Teleplot, dont il dépend, n'existe que sur la place de marché Microsoft. Son installation était quand même lancée, échouait sans rien dire, et le traceur restait muet. Un message explique maintenant la situation et propose d'ouvrir la page de téléchargement, pour une installation manuelle à partir du fichier VSIX. Sur Visual Studio Code officiel, rien ne change
...
- **Installed libraries were not detected as installed**: the extension resolved the sketchbook folder from the Windows registry / `preferences.txt` instead of asking the Arduino CLI for its real configuration. It now reads `arduino-cli config dump` (`directories.user` and `directories.data`), honoring a custom `arduino.arduinoCliConfigFile`
- Additional board manager URLs were never passed to the CLI (invalid `--build-property` root flag): third-party platforms (ESP32, STM32…) never appeared in the Board Manager. URLs are now passed via `--additional-urls` to `core install` and index updates
- "Refresh index" buttons actually refresh now: `core update-index` / `lib update-index` are used instead of a failing dummy install
- CLI exit code 1 is no longer treated as a successful install: real failures (network down, unknown package) are reported to the Board/Library Manager views instead of showing "Installed"
- `util cp()` had an inverted condition and never copied files: "Open example" on single-file examples created an empty folder
- A failed activation (e.g. offline at startup) was cached for the whole session, leaving all `arduino.*` commands dead until reload
- Debugging with `output: "."` in `arduino.yaml` could recursively delete the whole workspace — a strict subfolder guard was added
- OpenOCD scripts path is now quoted: debugging works for Windows user names containing spaces
- "Include Library" inserted no `#include` on Windows (glob with backslashes)
- "Upload using Arduino CLI" failed with `unknown flag: --library` when a custom library path was set (compile-only flags are no longer passed to `upload`)
- Board Manager view no longer leaks DeviceContext listeners on each opening (cascade of IntelliSense analyses)
- Library Manager no longer hangs forever on "Loading..." when the library index is corrupted or a `library.properties` lacks a `name` field; web view API errors now always return HTTP 500
- Command errors are now notified to the user instead of being silently logged
- `.pde` files are renamed to `.ino` only once (double listener caused ENOENT and closed the wrong editor)
- `where arduino-cli` returning several paths (choco + winget) no longer produces a garbage CLI path
- gdb `customLaunchSetupCommands` starting with `${file}` are now substituted (and all occurrences)
- Network interruption while downloading the Arduino CLI no longer leaves the progress notification stuck forever
- Marketplace README image was broken (angle-bracket link not rewritten by vsce)

### Changed

- Smaller VSIX: removed `todo.md`, README-only documentation image, Android serialport prebuilds, C++ sources of native modules, legacy webview fonts (~650 KB uncompressed). Fixed missing example-tree icons (`images/examples/**` was wrongly excluded)
- Removed redundant `onCommand:` / `onView:` activation events (auto-generated by VS Code since 1.75)
